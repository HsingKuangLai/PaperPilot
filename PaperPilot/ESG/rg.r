...
data$Prrice <- winsorize(data$Price)
data$RPA_Count_sq<-data$RPA_Count*data$RPA_Count


# Now, perform the Huber regression or any regression analysis using winsorized variables
model <- lm(Price ~ BVE + LNAT + NIS + KY + RPA + Year + Industry, data = data)
summary(model)

# Now, perform the Huber regression or any regression analysis using winsorized variables
model <- lm(Price ~ BVE + LNAT + NIS + RPA_Count + RPA_Count_sq + Year + KY +Industry, data = data)
summary(model)
