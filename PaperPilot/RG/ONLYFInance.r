...
data_PSM <- cbind(data, qq_data)

# Check for homoscedasticity using a plot of residuals vs. fitted values
plot(fitted(model), sqrt(abs(resid(model))), xlab = "Fitted values", ylab = "Square root of absolute residuals", main = "Scale-Location plot")
abline(h = 0, col = "red") # Add a horizontal line at y = 0

# Check for linearity using scatterplot of residuals vs. fitted values
