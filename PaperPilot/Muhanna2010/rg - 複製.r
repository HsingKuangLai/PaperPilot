...
data <- na.omit(data)
data$ROAN<-(data$ROAt+data$ROAt1)/2
...
data$ROAt1<-winsorize(data$ROAt1)
data$ROAN<-winsorize(data$ROAN)
...
model <- (lm(ROAN ~ RPA + ( ADV + RD  + ROA + SG  + S + BM ) + Year + Industry , data = data))
summary(model)
