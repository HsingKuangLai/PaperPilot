...
library(multcomp)
library(car)
# è®€??? CSV æª”æ?ˆï?Œå?? "#N/A" è½‰æ?›ç‚º??Ÿæ­£??? NAï¼ˆç¼ºå¤±å€¼ï??
data <- read.csv("base - C3.csv")
AQdata<-read.csv("SecondEq.csv")
AQdata<-AQdata[,c("KEY","AQ","IAQ","DAQ")]
IMRdata<-read.csv("data_IMR_complete.csv")
IMRdata<-IMRdata[,c("KEY","IMR")]
data<-inner_join(data,IMRdata,by="KEY")
...
data[1:16] <- lapply(data[1:16], factor)

# Apply the winsorize function to the remaining columns
data <- data %>%
  mutate(across(.cols = 17:ncol(data), .fns = ~winsorize(.)))
...
for (RM_proxy in RM_proxies) {
  ...
  control_vars_AM <- c("POST","AIPOST","MLPOST","AIPOST_RPA","MLPOST_RPA","RPA","POST_RPA","LEV","OCF","MTB","MS","INST","CYCLE","NOA","ZSCORE","CL", "ADJROA", "ADJROA_sq","SIZE","BIG4","YEAR")
  control_vars_RM <- c("POST","AIPOST","MLPOST","AIPOST_RPA","MLPOST_RPA","RPA","POST_RPA","LEV","OCF","MTB","MS","INST","CYCLE","NOA","ZSCORE","CL", "MTB", "ADJROA", "ADJROA_sq", "SIZE", "ADV","RD","YEAR")
  ...
  modelRM_formula <- as.formula(paste(RM_proxy, "~ AMhat +", paste(control_vars_RM, collapse=" + "),"+IMR"))
  ...
  modelAM_formula <- as.formula(paste(AM_proxy, "~   RMhat + ", paste(control_vars_AM, collapse=" + "),"+IMR"))
  ...
  glht_mod_AM <- glht(model = modelAM_endo, linfct = c("AIPOST1 + AIPOST_RPA1 = 0"))
  print(RM_proxy)
  print(summary(glht_mod_AM))
  glht_mod_RM <- glht(model = modelRM_endo, linfct = c("AIPOST1 +AIPOST_RPA1 = 0"))
  print(summary(glht_mod_RM))
  glht_mod_AM <- glht(model = modelAM_endo, linfct = c("MLPOST1 + MLPOST_RPA1 = 0"))
  print(RM_proxy)
  print(summary(glht_mod_AM))
  glht_mod_RM <- glht(model = modelRM_endo, linfct = c("MLPOST1 +MLPOST_RPA1 = 0"))
  print(summary(glht_mod_RM))
...
for (RM_proxy in RM_proxies) {
  ...
  # Define control variables , "ESG",, "Zscore"
  control_vars <- c("RPA_Ctd", "NOA", "INST", "MS", "LEV", "OCF", "MTB", "SG", "ADJROA", "ADJROA_sq", "ADV","SIZE", "Big4")
  control_vars_AM <- c("POST","AIPOST","MLPOST","LEV","OCF","MTB","MS","INST","CYCLE","NOA","ZSCORE","CL", "ADJROA", "ADJROA_sq","SIZE","BIG4","YEAR")
  control_vars_RM <- c("POST","AIPOST","MLPOST","LEV","OCF","MTB","MS","INST","CYCLE","NOA","ZSCORE","CL", "MTB", "ADJROA", "ADJROA_sq", "SIZE", "ADV","RD","YEAR")
