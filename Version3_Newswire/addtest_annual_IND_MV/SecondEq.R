...
mvdata<-read.csv("data_withNA.csv")
mvdata<-mvdata[,c("KEY","MV")]
alldata<-left_join(alldata,mvdata,by="KEY")
...
all_industries_data <- NULL
  ...
  for(year in unique_years) {
    ...
    IAQ_model <- lm(sigAQ ~ SIZE + sigCFO + sigS + LGCYCLE + NegNI + factor(Industry), data = year_data)
    ...
    file_name <- paste0("Year_", as.character(year), "_Regression_Results.txt")
    #sink(file_name)
