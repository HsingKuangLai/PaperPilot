...
for (Y_var in Y_vars) {
  for (X_var in X_vars) {
    ...
    formula <- as.formula(paste0(gsub("_pos|_neg", "", Y_var), " ~ RPA_Ctd  * ", X_var, " + LEV + OCF + MTB + ADJROA + ADJROA_sq + LGTA + Age + RD + ADV + ESG + Year"))
...
for (Y_var in Y_vars) {
  for (X_var in X_vars) {
    ...
    formula <- as.formula(paste0(Y_var, " ~ RPA_Ctd  * ", X_var, " + LEV + OCF + MTB + ADJROA + ADJROA_sq + LGTA + Age + RD + ADV + ESG + Year"))
