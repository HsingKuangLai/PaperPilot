...
for (Y_var in Y_vars) {
  for (X_var in X_vars) {
    if (substr(Y_var,1,3)!=substr(X_var,1,3)) {
      ...
      matched_data <- matchit(as.formula(paste0("RPA_Ctd ~ ", X_var, " + LEV + OCF + MTB + ADJROA  + ADJROA_sq  + LGTA + Age  + RD + ADV + ESG  + Big4 + Year")), data = data,link="probit", method = "nearest",distance = "glm")
...
stargazer::stargazer(models, type = "html", out = "PSMsq.html", 
