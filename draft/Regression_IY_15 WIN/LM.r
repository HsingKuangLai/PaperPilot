...
for (Y_var in Y_vars) {
  for (X_var in X_vars) {
    ...
    formula <- as.formula(paste0(gsub("_pos|_neg", "", Y_var), " ~ RPA_Ctd  * ", X_var, " + MS + LEV + OCF + MTB + ADJROA + LGTA + Age + RD + ESG + Big4 + Year"))
...
stargazer(models, type = "html", 
          se = se_list, 
          title = "AM-Regression Results with Clustered Standard Errors", out = "AM_MSF.html")
...
for (Y_var in Y_vars) {
  for (X_var in X_vars) {
    ...
    formula <- as.formula(paste0(Y_var, " ~ RPA_Ctd  * ", X_var, " + MS + LEV + OCF + MTB + ADJROA + LGTA + Age + RD+ ESG + Big4 + Year"))
...
stargazer(models, type = "html", 
          se = se_list, 
          title = "RM-Regression Results with Clustered Standard Errors", out = "RM_MSF.html")
