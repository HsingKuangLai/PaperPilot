...
for (Y_var in Y_vars) {
  for (X_var in X_vars) {
    if (substr(Y_var,1,3)!=substr(X_var,1,3)) {
      ...
      matched_data <- matchit(as.formula(paste0("RPA_Ctd ~ ", X_var, " + LEV + OCF + MTB + ADJROA  + ADJROA_sq  + LGTA + Age  + RD + ADV + ESG  + Big4 + Year + Industry")), data = data,link="logit", method = "nearest",distance = "glm")
      matched_data <- match.data(matched_data)
      
      # 3. Fit a linear model on the matched data
      model_formula <- as.formula(paste0(Y_var, " ~ RPA_Ctd * ", X_var, " + LEV + OCF + MTB + ADJROA + ADJROA_sq + LGTA + Age  + RD + ADV + ESG  + Big4 + Year + Industry"))
...
stargazer::stargazer(models, type = "html", out = "PSM_I.html", 
